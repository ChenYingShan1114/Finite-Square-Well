...
double V = -13.6 * eV / eV;
double a = 20 * Bohr_radius / Bohr_radius;
...
double ff_even(double E){
    ...
    tmp2 = sqrt(E - V);
    tmp3 = a * sqrt(2 * (E - V));
    return -0.5 * (1 / tmp1 + 1 / tmp2 * tan(nor * tmp3) + sqrt(2) * a * nor / cos(nor * tmp3) / cos(nor * tmp3));
}
...
double ff_odd(double E){
    ...
    tmp2 = sqrt(E - V);
    tmp3 = a * sqrt(2 * (E - V));
    return -0.5 * (1 / tmp1 - 1 / tmp2 / tan(nor * tmp3) + sqrt(2) * a * nor / sin(nor * tmp3) / sin(nor * tmp3));
}
        ...
        if (n % 2 == 0){
            ...
            for (int i = 0; i < 1000; i++){
                ...
                if (fabs((E_new - E_old)/E_old) < 1e-12){
                    ...
                    cout << " ---> eigenenergy of finite square well " << E_new << " (eV)" << endl;
